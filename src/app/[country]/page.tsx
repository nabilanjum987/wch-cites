...
interface EmergencyItem { label: string; number: string; color: string; }
...
const COUNTRIES: Record<string, Country> = {
  pakistan: {
    ...
    primaryColor: '#0C7A3D', secondaryColor: '#FFFFFF',
    ...
    climate: 'Varies — Desert to Alpine', majorFaith: 'Islam', majorFaithPct: 96,
  },
  india: {
    ...
    government: 'Federal Parliamentary Republic',
    primaryColor: '#FF9933', secondaryColor: '#138808',
    ...
    climate: 'Varies — Tropical to Alpine', majorFaith: 'Hinduism', majorFaithPct: 80,
  },
  'united-states': {
    ...
    primaryColor: '#B22234', secondaryColor: '#3C3B6E',
    ...
    climate: 'Varies — Arctic to Tropical', majorFaith: 'Christianity', majorFaithPct: 63,
  },
};
...
const PERSONALITIES: Record<string, Personality[]> = {
  PK: [
    { id: '1', name: 'Quaid-e-Azam Jinnah', profession: 'Father of the Nation', category: 'Leaders', photo: '', birthYear: 1876, achievements: 'Founded Pakistan on August 14, 1947' },
  ...
  US: [
    ...
    { id: '2', name: 'Michael Jordan', profession: 'Athlete', category: 'Sports', photo: '', birthYear: 1963, achievements: '6x NBA Champion' },
...
const LANDMARKS: Record<string, Landmark[]> = {
  PK: [
    ...
    { id: '6', name: 'Faisal Mosque', type: 'Religious', city: 'Islamabad', image: 'https://images.pexels.com/photos/208371/pexels-photo-208371.jpeg?auto=compress&cs=tinysrgb&w=600', unesco: false, era: 'Modern 1986' },
  ],
  IN: [
...
const RESOURCES: Record<string, Resource[]> = {
  PK: [
    ...
    { name: 'Copper & Gold', icon: '🟡', description: 'Reko Diq in Balochistan — one of the largest undeveloped copper-gold deposits on earth.', rank: 'World top 5 deposit' },
    ...
    { name: 'Water', icon: '🌊', description: 'Indus River system — one of the great river systems of the world, feeding Pakistan\'s agriculture.', rank: 'Top 10 river systems' },
  ],
  IN: [
    ...
    { name: 'Diamonds', icon: '💎', description: 'Historic diamond mining — Golconda diamonds were world-famous.', rank: 'Historically significant' },
  ],
  US: [
    { name: 'Oil & Gas', icon: '🛢️', description: "World's largest oil producer since 2018 through the shale revolution.", rank: 'World #1 producer' },
...
const EMERGENCY: Record<string, EmergencyItem[]> = {
  PK: [
...
export default function CountryPage() {
  ...
  const [activeHolidayType, setActiveHolidayType] = useState('All');
  ...
  useEffect(() => {
    ...
    setPersonalities(PERSONALITIES[countryData.code] || []);
    setLandmarks(LANDMARKS[countryData.code] || []);
    setEconomy(ECONOMIES[countryData.code] || null);
    setTeams(NATIONAL_TEAMS[countryData.code] || []);
    setHolidays(HOLIDAYS[countryData.code] || []);
    setNeighbors(NEIGHBORS[countryData.code] || []);
    setRates(RATES[countryData.code] || []);
    setResources(RESOURCES[countryData.code] || []);
  ...
  const accent = country.primaryColor;
  const bg = '#0a0f1e';
  const cardBg = 'rgba(255,255,255,0.04)';
  const cardBorder = `${accent}30`;

  const personalityCategories = ['All', ...Array.from(new Set(personalities.map(p => p.category)))];
  const filteredPersonalities = activeCategory === 'All' ? personalities : personalities.filter(p => p.category === activeCategory);
  const holidayTypes = ['All', ...Array.from(new Set(holidays.map(h => h.type)))];
  const filteredHolidays = activeHolidayType === 'All' ? holidays : holidays.filter(h => h.type === activeHolidayType);
    ...
    <div className="min-h-screen" style={{ backgroundColor: bg }}>
      ...
      <div className="relative z-10 h-72 md:h-96 overflow-hidden">
        ...
        <div className="absolute bottom-6 left-6 md:left-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-black" style={{ color: accent }}>{country.name}</h1>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 pb-20 space-y-14 pt-8">
        ...
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Globe} title="Country Facts" accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
            {generateFactsParagraph(country.name, country.capital)}
          </p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {[
              { label: 'Population', value: formatNumber(country.population), icon: '👥' },
              { label: 'Area', value: `${formatNumber(country.area)} km²`, icon: '🗺️' },
        ...
        {economy && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            ...
            <div className="rounded-2xl p-5 border flex flex-wrap items-center justify-between gap-6"
              style={{ backgroundColor: 'rgba(0,0,0,0.4)', borderColor: `${accent}30` }}>
              {[
                { label: 'Currency', value: economy.currencyRate },
                { label: 'GDP', value: economy.gdp },
                { label: 'Inflation', value: economy.inflation },
                { label: 'Growth Rate', value: economy.growth },
              ].map(item => (
              ...
              <div className="text-center">
                <div className="text-gray-500 text-xs mb-1">Next Prayer (Capital)</div>
                <div className="font-bold text-sm" style={{ color: accent }}>Maghrib 6:45 PM</div>
              </div>
            </div>
        ...
        {/* ── 3. WEATHER ── */}
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Thermometer} title={`${country.name} Weather Today`} accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {cities.map(city => (
              <Link key={city.slug}
                href={`/${country.slug}/${city.provinceSlug}/${city.slug}/weather`}
        ...
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          ...
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
            ...
            prayer times, and sacred sites. The section below shows today's multi-faith calendar dates
            and the next prayer time for the most widely observed faith in {country.name}.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-2xl p-5 border" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
              <div className="text-white font-semibold text-sm mb-4">Religious Population</div>
              {[
                ...
                <div key={r.faith} className="mb-3">
                  ...
                  <div className="h-1.5 rounded-full bg-white/10">
                    <div className="h-1.5 rounded-full" style={{ width: `${r.pct}%`, backgroundColor: accent }} />
                  </div>
                </div>
              ))}
            </div>
            <div className="md:col-span-2 grid grid-cols-2 gap-3">
              {[
                ...
                <div key={f.faith} className="rounded-xl p-4 border" style={{ backgroundColor: cardBg, borderColor: `${accent}25` }}>
        ...
        {/* ── 5. MARKET RATES ── */}
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Coins} title={`${country.name} Market Rates Today`} accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
            Gold, silver, currency exchange rates, and fuel prices for {country.name} as of today.
            These figures are the most searched daily reference points for traders, travellers,
            businesses, and households across the country. For a full 30-day history and trend
            charts, visit the dedicated Rates page.
          </p>
          ...
          <Link href={`/${country.slug}/rates`} className="inline-flex items-center gap-2 text-sm font-medium" style={{ color: accent }}>
        ...
        {/* ── 6. NEWS ── */}
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Newspaper} title={`${country.name} Today`} accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
            A snapshot of today's most important headlines from {country.name} and global coverage
            about the country. The national section covers politics, economy, sports, and culture
            happening inside {country.name} right now. The global section shows what international
            media is reporting about {country.name} today.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div className="rounded-2xl p-5 border" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
              ...
              <div className="space-y-3">
                {['IMF programme review — decision expected this month',
                  `${country.name} diplomatic developments in the region`,
                  `International economic outlook for ${country.name} 2025`].map((h, i) => (
            ...
            <div className="rounded-2xl p-5 border" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
              ...
              <div className="space-y-3">
                {['PM chairs economic committee meeting',
                  country.code === 'PK' ? 'PSL 2025 playoffs schedule announced' : 'National sports league latest',
        ...
        {provinces.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={MapPin as any} title={`Explore ${country.name} by Province`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              {generateProvincesParagraph(country.name)}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {provinces.map(prov => (
                <Link key={prov.slug} href={`/${country.slug}/${prov.slug}`}
                  ...
                  <div className="text-gray-500 text-xs mt-1">Pop: <span className="text-gray-300">{formatNumber(prov.population)}</span> · Area: <span className="text-gray-300">{formatNumber(prov.area)} km²</span></div>
        ...
        {economy && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={TrendingUp} title={`${country.name} Economy`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              {generateEconomyDashboardParagraph(country.name)}
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
              {[
                { label: 'GDP', value: economy.gdp, color: '#34d399' },
                { label: 'Growth Rate', value: economy.growth, color: '#60a5fa' },
                { label: 'Inflation', value: economy.inflation, color: '#f87171' },
                { label: 'Unemployment', value: economy.unemployment, color: '#fb923c' },
              ].map(stat => (
                <div key={stat.label} className="rounded-2xl p-5 border text-center"
                  style={{ backgroundColor: cardBg, borderColor: `${stat.color}30` }}>
            ...
            {economy.exports && (
              <div className="rounded-2xl p-5 border mb-4" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
        ...
        {personalities.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Users} title={`${country.name}'s Finest`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
        ...
        {landmarks.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Mountain} title={`${country.name} Must See`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
        ...
        {/* ── 11. HERITAGE PRODUCTS ── */}
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Gem} title={`${country.name} Heritage Products`} accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
            ...
            the community, and the generation that made them. From intricate embroidery and pottery
            to sports goods and surgical instruments, {country.name}'s heritage products are sold
            in markets across the world and represent a living tradition as much as an economic activity.
          </p>
          <div className="flex gap-3 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none' }}>
            {(country.code === 'PK' ? [
              ...
              { name: 'Pashmina', emoji: '🧣', origin: 'Kashmir' },
              { name: 'Banarasi Silk', emoji: '🥻', origin: 'Varanasi' },
              { name: 'Madhubani Art', emoji: '🎨', origin: 'Bihar' },
              { name: 'Blue Pottery', emoji: '🏺', origin: 'Jaipur' },
              { name: 'Kanjivaram Silk', emoji: '🥻', origin: 'Tamil Nadu' },
            ] : [
              { name: 'Navajo Textiles', emoji: '🧶', origin: 'Southwest' },
              { name: 'Pueblo Pottery', emoji: '🏺', origin: 'New Mexico' },
              { name: 'Cherokee Baskets', emoji: '🧺', origin: 'Southeast' },
              { name: 'Alaskan Ivory', emoji: '🦴', origin: 'Alaska' },
            ]).map(prod => (
              <div key={prod.name} className="flex-shrink-0 min-w-[130px] rounded-2xl p-4 border text-center"
                style={{ backgroundColor: cardBg, borderColor: `${accent}25` }}>
        ...
        {resources.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Leaf} title={`${country.name} Natural Resources`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              {country.name} holds substantial natural wealth that has shaped its history and continues
              to drive its economic potential. From mineral deposits and energy resources to river
              systems and agricultural land, these resources represent both the foundation of the
              current economy and the basis for long-term development.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {resources.map(res => (
        ...
        {teams.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Trophy} title={`${country.name} in Sport`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
                ...
                <div key={team.sport} className="rounded-2xl p-5 border"
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-xs px-2 py-0.5 rounded-full font-semibold"
                      style={{ backgroundColor: `${accent}20`, color: accent }}>{team.ranking}</div>
                  </div>
        ...
        {foods.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Utensils} title={`${country.name} Famous Foods`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              ...
              These are the dishes {country.name} is known for across the world.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {foods.map(food => (
        ...
        {holidays.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Calendar} title={`${country.name} Holidays & Events`} accent={accent} />
            <p className="text-gray-400 text-sm leading-relaxed mb-5">
              {generateHolidaysParagraph(country.name)}
            </p>
            <div className="flex gap-2 flex-wrap mb-5">
              {holidayTypes.map(t => (
                <button key={t} onClick={() => setActiveHolidayType(t)}
                  className="px-4 py-1.5 rounded-lg text-sm font-medium transition-all border"
                  style={activeHolidayType === t
                    ? { backgroundColor: accent, color: '#fff', borderColor: accent }
                    : { backgroundColor: `${accent}15`, color: accent, borderColor: `${accent}40` }}>
                  {t}
                </button>
              ...
              {filteredHolidays.map(h => (
                <div key={h.name} className="rounded-xl p-4 border flex items-center justify-between"
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full"
                    style={{ backgroundColor: `${accent}20`, color: accent }}>{h.type}</span>
                </div>
              ))}
            </div>
            <p className="text-gray-500 text-sm leading-relaxed mt-5">
        ...
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={ShieldAlert} title={`${country.name} Emergency Contacts`} accent={accent} />
          <p className="text-gray-400 text-sm leading-relaxed mb-5">
        ...
        {neighbors.length > 0 && (
          <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <SectionHeader icon={Globe} title={`${country.name} Neighbouring Countries`} accent={accent} />
        ...
        {/* ── 18. EXPLORE LINKS ── */}
        <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <SectionHeader icon={Globe} title={`Explore ${country.name} in Detail`} accent={accent} />
          <div className="flex flex-wrap gap-2">
            {[
              { label: 'Provinces', href: `/${country.slug}` },
              { label: 'Weather', href: `/${country.slug}/${provinces[0]?.slug || 'province'}/islamabad/weather` },
              { label: 'Prayer Times', href: `/${country.slug}/${provinces[0]?.slug || 'province'}/islamabad/prayer-times` },
              { label: 'Rates', href: `/${country.slug}/rates` },
              { label: 'News', href: `/${country.slug}/news` },
              { label: 'Famous People', href: `/${country.slug}/personalities` },
              { label: 'Famous Places', href: `/${country.slug}/places` },
              { label: 'Economy', href: `/${country.slug}/economy` },
              { label: 'Heritage Products', href: `/${country.slug}/heritage-products` },
              { label: 'Events', href: `/${country.slug}/events` },
              { label: 'Compare', href: `/compare?a=${country.slug}` },
            ].map(link => (
