...
// ─── 19. MULTI-FAITH CALENDAR ──────────────────────────────────────────

export function generateMultiFaithParagraph(city: string, country: string): string {
  const { month, year } = now();
  return `Today in ${city}, multiple calendar systems run simultaneously — each marking a different date for different communities, each rooted in a distinct astronomical or historical tradition. The Islamic Hijri calendar, followed by ${city}'s majority Muslim population, tracks lunar months from the Prophet's migration in 622 CE. The Gregorian calendar, used for civic and international purposes across ${country}, is a solar calendar adopted globally from the 16th century. The Hebrew calendar, observed by Jewish communities worldwide, is a lunisolar system dating to antiquity. The Hindu Panchang, followed by Hindu residents and communities across South Asia, uses a complex system of solar months, lunar days, and planetary positions. The Buddhist Era calendar, used across Southeast and East Asia, counts from the year of the Buddha's parinirvana. The Nanakshahi calendar, introduced by the Sikh community in 2003, is a solar calendar tied to events in Sikh history. This side-by-side view in ${month} ${year} reflects ${city}'s position as a city where multiple faith traditions coexist, each carrying their own sense of time, sacred dates, and annual rhythm.`;
}

export function generateMultiFaithAfter(city: string): string {
  return `These calendar systems are not merely timekeeping conventions — they determine when communities fast, when they celebrate, when they rest, and how they mark the passage of life from birth to death. In a city like ${city}, the overlap between these calendars means that at any point in the year, some community is observing a fast, a festival, or a sacred day. The multi-faith calendar view above is updated daily, ensuring that whatever date today falls on in each tradition is accurately reflected. The dedicated Prayer Times page for ${city} provides full details for Islamic prayer times, while the Events page captures upcoming religious observances across all traditions.`;
}
