    ...
    <div className="min-h-screen bg-transparent/4">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <div className="flex items-center gap-2 mb-4">
            <Activity size={18} style={{ color: primaryColor }} />
            <h2 className="font-bold text-white">Economic Health Score — {countryName}</h2>
          </div>
          {(() => {
            const score = Math.max(0, Math.min(100, Math.round(
              (eco.gdpGrowth > 0 ? Math.min(eco.gdpGrowth * 5, 30) : 0) +
              (eco.inflation < 5 ? 25 : eco.inflation < 10 ? 15 : eco.inflation < 20 ? 5 : 0) +
              (eco.unemployment < 5 ? 25 : eco.unemployment < 10 ? 15 : eco.unemployment < 20 ? 5 : 0) +
              (eco.debtToGdp < 40 ? 20 : eco.debtToGdp < 60 ? 12 : eco.debtToGdp < 80 ? 5 : 0)
            )));
            const label = score >= 75 ? 'Strong' : score >= 50 ? 'Moderate' : score >= 25 ? 'Fragile' : 'Critical';
            ...
            return (
              <div className="flex flex-col items-center py-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full mt-2">
                  {[
                    { label: 'GDP Growth', value: `${eco.gdpGrowth}%`, positive: eco.gdpGrowth > 0 },
                    { label: 'Inflation', value: `${eco.inflation}%`, positive: eco.inflation < 5 },
                    { label: 'Unemployment', value: `${eco.unemployment}%`, positive: eco.unemployment < 5 },
                    { label: 'Debt/GDP', value: `${eco.debtToGdp}%`, positive: eco.debtToGdp < 60 },
                  ].map(v => (
                    <div key={v.label} className="rounded-xl p-3 border border-white/8 text-center">
                      <div className="text-xs text-gray-500 mb-1">{v.label}</div>
                      <div className="font-bold text-sm" style={{ color: v.positive ? '#10B981' : '#EF4444' }}>{v.value}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-white">Cost of Living — {cityName}</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-gray-500 border-b border-white/8">
                  <th className="text-left py-2 pr-4 font-normal">Item</th>
                  <th className="text-right py-2 pr-4 font-normal">Est. Cost (USD)</th>
                  <th className="text-right py-2 font-normal">vs World Avg</th>
                </tr>
              </thead>
              <tbody>
                {[
                  { item: 'Monthly rent (1BR city centre)', cost: Math.round(eco.gdpPerCapita * 0.012), vs: -35 },
                  { item: 'Monthly groceries (single person)', cost: Math.round(eco.gdpPerCapita * 0.006), vs: -42 },
                  { item: 'Monthly transport pass', cost: Math.round(eco.gdpPerCapita * 0.001), vs: -55 },
                  { item: 'Meal, inexpensive restaurant', cost: Math.round(eco.gdpPerCapita * 0.0003), vs: -60 },
                  { item: 'Monthly utilities (85m² apt)', cost: Math.round(eco.gdpPerCapita * 0.002), vs: -30 },
                  { item: 'Internet (60 Mbps)', cost: Math.round(eco.gdpPerCapita * 0.0005), vs: -45 },
                ].map(r => (
                  <tr key={r.item} className="border-b border-white/5">
                    <td className="py-2.5 pr-4 text-gray-300">{r.item}</td>
                    <td className="py-2.5 pr-4 text-right text-white font-medium">${r.cost}</td>
                    <td className="py-2.5 text-right text-green-400 text-xs">{r.vs}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-gray-600 mt-3">Estimates based on GDP per capita ratios. Actual costs vary by neighbourhood and lifestyle.</p>
        </div>
        ...
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-white">Industries — {countryName}</h2>
          </div>
          <div className="space-y-3">
            {[
              { sector: 'Services', pct: Math.round(55 + eco.gdpGrowth), color: '#3B82F6' },
              { sector: 'Industry & Manufacturing', pct: Math.round(25 - eco.gdpGrowth * 0.5), color: '#F59E0B' },
              { sector: 'Agriculture', pct: Math.round(20 - eco.gdpGrowth * 0.3), color: '#10B981' },
            ].map(s => (
              <div key={s.sector}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-300">{s.sector}</span>
                  <span className="text-white font-medium">{Math.max(5, Math.min(80, s.pct))}%</span>
                </div>
                <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full rounded-full" style={{ width: `${Math.max(5, Math.min(80, s.pct))}%`, backgroundColor: s.color }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-white">Typical Salaries — {cityName}</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              { role: 'Software Engineer', monthly: Math.round(eco.gdpPerCapita * 0.004) },
              { role: 'Teacher (public school)', monthly: Math.round(eco.gdpPerCapita * 0.0015) },
              { role: 'Doctor (general)', monthly: Math.round(eco.gdpPerCapita * 0.005) },
              { role: 'Factory Worker', monthly: Math.round(eco.gdpPerCapita * 0.0008) },
              { role: 'Bank Officer', monthly: Math.round(eco.gdpPerCapita * 0.003) },
              { role: 'Civil Servant (mid-level)', monthly: Math.round(eco.gdpPerCapita * 0.002) },
            ].map(s => (
              <div key={s.role} className="rounded-xl p-3 border border-white/8 flex justify-between items-center">
                <span className="text-gray-300 text-sm">{s.role}</span>
                <span className="text-white font-semibold text-sm">${s.monthly}/mo</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-gray-600 mt-3">Estimated from GDP per capita ratios. Actual salaries vary significantly.</p>
        </div>

        {/* ── DID YOU KNOW ── */}
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <div className="flex items-center gap-2 mb-4">
            <AlertTriangle size={18} style={{ color: primaryColor }} />
            <h2 className="font-bold text-white">Did You Know — {countryName} Economy</h2>
          </div>
          <div className="space-y-3">
            {[
              `${countryName}'s GDP per capita of $${eco.gdpPerCapita.toLocaleString()} places it among ${eco.gdpPerCapita > 10000 ? 'upper-middle' : eco.gdpPerCapita > 4000 ? 'lower-middle' : 'low'} income countries by World Bank classification.`,
              `At the current growth rate of ${eco.gdpGrowth}%, ${countryName}'s economy would double in approximately ${eco.gdpGrowth > 0 ? Math.round(70 / eco.gdpGrowth) : '—'} years (Rule of 70).`,
              `${countryName}'s remittance inflows are a significant source of foreign exchange, often exceeding foreign direct investment in developing economies.`,
              `With ${eco.unemployment}% unemployment, ${countryName} has approximately ${Math.round(eco.laborForce * eco.unemployment / 100 * 1000000).toLocaleString()} people actively seeking work.`,
            ].map((fact, i) => (
        ...
        {/* ── AFFILIATES ── */}
        <div className="bg-transparent rounded-2xl border border-white/8 p-6">
          <h3 className="font-bold text-white mb-4">Useful Tools & Resources</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <a href="https://tradingeconomics.com" target="_blank" rel="noopener noreferrer"
              className="rounded-xl p-4 border border-white/8 hover:border-white/20 transition group">
              <div className="text-2xl mb-2">📊</div>
              <div className="text-white font-medium text-sm group-hover:text-blue-400 transition">TradingEconomics</div>
              <div className="text-gray-500 text-xs mt-1">Live macro data & charts</div>
            </a>
            <a href="https://www.coursera.org/search?query=economics" target="_blank" rel="noopener noreferrer"
              className="rounded-xl p-4 border border-white/8 hover:border-white/20 transition group">
              <div className="text-2xl mb-2">🎓</div>
              <div className="text-white font-medium text-sm group-hover:text-blue-400 transition">Coursera Economics</div>
              <div className="text-gray-500 text-xs mt-1">Free & paid online courses</div>
            </a>
            <a href="https://wise.com" target="_blank" rel="noopener noreferrer"
              className="rounded-xl p-4 border border-white/8 hover:border-white/20 transition group">
              <div className="text-2xl mb-2">💱</div>
              <div className="text-white font-medium text-sm group-hover:text-blue-400 transition">Wise (TransferWise)</div>
              <div className="text-gray-500 text-xs mt-1">Low-cost international transfers</div>
            </a>
          </div>
        </div>
